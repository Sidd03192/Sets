 ...
 */
 ...
 public class SortedSet<E extends Comparable<? super E>> extends AbstractSet<E> {
     ...
     public SortedSet(ISet<E> other) {
         ...
         // do merge sort
         doMergeSort(0, size() - 1, myCon, new ArrayList<>());
     }
     
     // helper method which sorts and array using merge sort algorithm.
     private void doMergeSort(int left, int right, ArrayList<E> data, ArrayList<E> temp) {
         // code for merge sort from class slides
         if (left < right) {
 
             int mid = (left + right) / 2;
             // partition into two "technically"
             doMergeSort(left, mid, data, temp);
             doMergeSort(mid + 1, right, data, temp);
             //now that everything is sorted, we can start putting the "arrayLists" together.
             mergeArrayLists(left, right, mid + 1, data, temp);
 
         }
     }
     // helper method for doMergeSort which helps sort merge each sorted virtual array
     // merges two sorted arrayLists.
     private void mergeArrayLists(int leftPos, int rightEnd, int rightPos, ArrayList<E> data, ArrayList<E> temp) {
         ...
         for (int i = 0; i < numElements; i++) {
             data.set(rightEnd - numElements + 1 + i, temp.get(i));
         }
     }
     ...
     public boolean add(E item) {
         ...
         // if positive then alr contain
         if (contains(item)) {
             return false;
         }
         int index = 0;
         while (index < size() && myCon.get(index).compareTo(item) < 0) {
             index++; 
         }
     ...
     public boolean contains(E item) {
         ...
         return containsHelper(item, 0, size());
     }
     ...
     **/ 
     private boolean containsHelper(E item, int start, int stop) {
         ...
         if (myCon.get(pos).compareTo(item) > 0) {
             // looks to the left
             return containsHelper(item, start, pos);
         } else {
             // looks to the rights
             return containsHelper(item, pos + 1, stop);
         }
     }
